...
def normalize_data(data: dict) -> dict[str | datetime]:
    ...
    location: dict[str] = {
        'location_name': (
            str(
                'Evento Online'
                if address.get('address_num') == '0'
                else address.get('name', 'Local não informado')
                ).strip().title()
        ),
        'city': (
            str(
                'Evento Online'
                if address.get('address_num') == '0'
                else address.get('city', 'Cidade não informada')
                ).strip().title()
        )
    }
    ...
    category_prim: str = (
        str(
            data.get('category_prim', {}).get('name', '')
            ).strip().title()
    )
    category_sec: str = (
        str(
            data.get('category_sec', {}).get('name', '')
            ).strip().title()
    )
